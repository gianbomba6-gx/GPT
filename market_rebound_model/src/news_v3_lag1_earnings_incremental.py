...
def evaluate(base: pd.DataFrame, selected_rows: pd.DataFrame, label: str, n_boot: int, cost_bps: float) -> dict:
    ...
    result = {
        "set": label, "n_base": len(base), "n_eligible": len(eligible), "n_selected": int(len(selected)),
        "mean_base": float(base["next_ret"].mean()) if len(base) else 0.0,
        "mean_selected_gross": 0.0, "mean_selected_net": 0.0, "delta_mean": 0.0,
        "ci_low": 0.0, "ci_high": 0.0, "cost_bps": cost_bps,
    ...
    if not mask.any():
        result.update({"mean_selected_gross": np.nan, "mean_selected_net": np.nan, "delta_mean": np.nan,
                       "ci_low": np.nan, "ci_high": np.nan, "status": "INVALID_SELECTION_ID_ALIGNMENT"})
        return result
    values = base["next_ret"].to_numpy(float)
    delta, lo, hi = bootstrap_delta(values, mask, n_boot, 42)
    mean_sel = float(values[mask].mean())
    result.update({"mean_selected_gross": mean_sel, "mean_selected_net": mean_sel - cost_bps / 10000.0,
                   "delta_mean": delta, "ci_low": lo, "ci_high": hi, "status": "OK"})
    return result


def annual_rows(base: pd.DataFrame, selected_rows: pd.DataFrame, filter_name: str) -> pd.DataFrame:
    out = []
    selected_rows = selected_rows[selected_rows["filter"] == filter_name].copy()
    for symbol, b in base.groupby("symbol", sort=True):
        b = b.copy(); b["year"] = pd.to_datetime(b["Date"]).dt.year
        s = selected_rows[selected_rows["symbol"] == symbol].copy(); s["year"] = pd.to_datetime(s["Date"]).dt.year
        for year, by in b.groupby("year", sort=True):
            by = by[by["next_ret"].notna()].copy()
            sy = s[(s["year"] == year) & s["eligible"] & s["selected"] & s["next_ret"].notna()].copy()
            mean_base = float(by["next_ret"].mean()) if len(by) else 0.0
            mean_sel = float(sy["next_ret"].mean()) if len(sy) else 0.0
            out.append({"symbol": symbol, "year": int(year), "filter": filter_name, "n_base": len(by),
                        "n_selected": len(sy), "mean_base": mean_base, "mean_selected_gross": mean_sel,
...
def main() -> None:
    ...
    if scored.empty or "_row_id" not in scored.columns:
        raise SystemExit("Earnings scoring did not preserve V1 row ids")
    if set(scored["_row_id"]) != set(base["_row_id"]):
        raise SystemExit("Earnings scoring changed V1 row-id coverage")
    ...
    for symbol, b in base.groupby("symbol", sort=True):
        ...
        for frac, sel_label in ((0.25, "earnings_top25"), (0.50, "earnings_top50")):
            for direction in ("normal", "inverted"):
                if news.empty:
                    reports.append({"set": f"{symbol}: {sel_label}: {direction}", "n_base": len(b[b["next_ret"].notna()]),
                ...
                y = x.copy(); y["filter"] = sel_label; y["direction"] = direction; parts.append(y)
        reports.append({"set": f"{symbol}: V1 top20 baseline", "n_base": len(b[b["next_ret"].notna()]),
    ...
    stability = pd.concat([
        annual_rows(base, rows_out[rows_out["direction"] == "inverted"], "earnings_top25"),
        annual_rows(base, rows_out[rows_out["direction"] == "inverted"], "earnings_top50")
    ], ignore_index=True)

    numeric_cols = ["n_base", "n_news_candidates", "n_eligible", "n_selected", "mean_base", "mean_selected_gross",
                    "mean_selected_net", "delta_mean", "ci_low", "ci_high", "cost_bps"]
    if report[numeric_cols].apply(pd.to_numeric, errors="coerce").isna().any().any():
        raise SystemExit("Invalid earnings incremental numeric result")
    if (report["n_selected"] > report["n_eligible"]).any() or (report["ci_low"] > report["ci_high"]).any():
        raise SystemExit("Invalid earnings incremental counts or CI")
    ok_st = stability[stability["status"].eq("OK")]
    if not ok_st.empty and ok_st[["n_base", "n_selected", "mean_base", "mean_selected_gross", "delta_mean"]].apply(pd.to_numeric, errors="coerce").isna().any().any():
        raise SystemExit("Invalid earnings stability numeric result")

    Path(args.out).parent.mkdir(parents=True, exist_ok=True)
    report.to_csv(args.out, index=False); rows_out.to_csv(args.out_rows, index=False); stability.to_csv(args.out_stability, index=False)
    print("NEWS V3 LAG1 EARNINGS INCREMENTAL VALUE")
    print(report.to_string(index=False))
    print("NEWS V3 LAG1 EARNINGS STABILITY")
    print(stability.to_string(index=False))
    print(f"Saved {args.out}"); print(f"Saved {args.out_rows}"); print(f"Saved {args.out_stability}")
